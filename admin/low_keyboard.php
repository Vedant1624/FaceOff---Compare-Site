...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["add_product"])) {
    ...
    $productNoAntiGhosting = $_POST["NoAntiGhosting"];
    ...
    $sql = "INSERT INTO lowkeyboard (Id, Name, Price, Weight, Warranty, Connectivity, Backlit, RGB, KeyType, LockKey, TKL, Orientation, AntiGhosting, NoAntiGhosting, CableLength, CableType, Nokey, KeyLife, Origin, Length, Width, Height,Url)
            VALUES ('$productId', '$productName', '$productPrice', '$productWeight', '$productWarranty', '$productConnectivity', '$productBacklit', '$productRGB', '$productKeyType', '$productLockKey', '$productTKL', '$productOrientation', '$productAntiGhosting', '$productNoAntiGhosting', '$productCableLength', '$productCableType', '$productNokey', '$productKeyLife', '$productOrigin', '$productLength', '$productWidth', '$productHeight','$productUrl')";



    if ($conn->query($sql) === TRUE) {
        echo "<script>window.location.href='budget_keyboard.php'; alert('Record Added Successfully')</script>";
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["update_product"])) {
    ...
    $sql = "UPDATE lowkeyboard SET 
            ...
            NoAntiGhosting = '$productNoAntiGhosting', 
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        echo "<script>window.location.href='budget_keyboard.php'; alert('Record updated Successfully')</script>";
    } else {
        echo "Error: " . $sql . "<br>" . $conn->error;
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["delete_product"])) {
    ...
    $sql = "DELETE FROM lowkeyboard WHERE Id = '$productIdToDelete'";

    // Execute the delete query
    if ($conn->query($sql) === TRUE) {
        echo "<script>window.location.href='budget_keyboard.php'; alert('Record Deleted Successfully')</script>";
    } else {
        echo "Error: " . $sql . "<br>" . $conn->error;
    }
}
...
<body>
    ...
    <section>
        <h1>Admin Panel | Budget Keyboard</h1>
